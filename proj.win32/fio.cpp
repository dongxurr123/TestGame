    ...
    int       nCmdShow)
{
    ...
    return FioPlayer::createAndRun();
}

int FioPlayer::createAndRun(void)
{
    FioPlayer *host = FioPlayer::sharedInstance();
    int ret = host->run();
    FioPlayer::purgeSharedInstance();
    return ret;
}

FioPlayer *FioPlayer::s_sharedInstance = NULL;

FioPlayer *FioPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new FioPlayer();
    }
    return s_sharedInstance;
}

void FioPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
int FioPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "fio-player");
...
void FioPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void FioPlayer::createViewMenu(void)
{
...
void FioPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void FioPlayer::updateWindowTitle(void)
{
}

void FioPlayer::relaunch(void)
{
...
void FioPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void FioPlayer::onFileOpenProject(void)
{
...
void FioPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"FioPlayer");
...
void FioPlayer::onFileProjectConfig(void)
{
...
void FioPlayer::onFileRelaunch(void)
{
    relaunch();
}

void FioPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void FioPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void FioPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void FioPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void FioPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT FioPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    FioPlayer *host = FioPlayer::sharedInstance();
...
INT_PTR CALLBACK FioPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
